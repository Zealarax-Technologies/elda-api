...
async function UpdateContactDetailsRequest(req, res, next)
{
    ...
    if(value.title)
    {
        if (await models.ContactDetails.findOne({ where: { title: value.title, uuid: {[Op.ne]: req.params.id} } })) {
            return ApiResponses(res, 422, 'Validation Errors', FormatError('title', 'A record with this title already exists'));
        }
    }

    req.contactData = value;
    next();
}

module.exports = UpdateContactDetailsRequest;