...
async function CreateAdmissionDetailsRequest(req, res, next)
{
    ...
    const [existingTitle] = await Promise.all([
        models.AdmissionDetails.findOne({ where: { title: value.title } }),
    ]);
...
module.exports = CreateAdmissionDetailsRequest;