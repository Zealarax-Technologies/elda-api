...
async function UpdateSpecialEnquiryRequest(req, res, next)
{
    ...
    if(value.title)
    {
        if (await models.SpecialEnquiry.findOne({ where: { title: value.title, uuid: {[Op.ne]: req.params.id} } })) {
...
module.exports = UpdateSpecialEnquiryRequest;